    ...
    try:
        ...
        if usage_tracker:
            if usage is not None and usage.input_tokens is not None:
                # Prefer real token counts reported by the provider.
                ...
                # Fall back to a word-count estimate if the provider didn't report usage.
                input_text = content + (additional_context or "") + (instructions or "")
                estimated_input = usage_tracker.estimate_tokens(input_text) + 500
