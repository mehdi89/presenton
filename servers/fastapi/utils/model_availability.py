...
async def check_llm_and_image_provider_api_or_model_availability():
    ...
    if not can_change_keys:
        ...
        if not skip_image_validation:
            ...
            # _check_image_provider_configuration() (shared with the non-can_change_keys
            # path above) doesn't know about our custom Azure image providers;
            # validate those here so a misconfigured deployment fails fast at startup.
            selected_image_provider = get_selected_image_provider()
            if selected_image_provider == ImageProvider.AZURE_FLUX:
                if not get_azure_flux_api_key_env():
                    raise Exception("AZURE_FLUX_API_KEY must be provided")
                if not get_azure_flux_endpoint_env():
